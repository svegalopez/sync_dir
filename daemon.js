...
const chokidar = require("chokidar"); // More powerful than fs.watch
...
async function main() {
  ...
  // 1. Robustness: Check if the directory exists
  if (!fs.existsSync(watchDir)) {
    console.error(`Directory not found: ${watchDir}`);
    process.exit(1);
  }

  console.log(`Daemon watching directory: ${watchDir}`);

  // 2. File Watcher Setup (Chokidar)
